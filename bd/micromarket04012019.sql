...
-- Volcando datos para la tabla marketbbdd.compra_r: ~40 rows (aproximadamente)
...
INSERT INTO `compra_r` (`id_compra`, `nro_recibo`, `total`, `total_literal`, `fecha`, `id_cliente`, `id_usuario`, `estado`) VALUES
	...
	(40, '40', 420, 'CUATROCIENTOS VEINTE con 00/100 Bs.', '2018-12-18 22:55:10', 209, 5, NULL),
	(41, '41', 48, 'CUARENTA Y OCHO con 00/100 Bs.', '2019-01-04 19:39:10', 105, 5, NULL);
...
WHERE cl.id_cliente = c.id_cliente AND date(c.fecha) BETWEEN fecha_a and fecha_b;
END//
DELIMITER ;
...
CREATE TABLE IF NOT EXISTS `producto_etiquetado` (
  ...
  CONSTRAINT `FK_COMPRA_DET` FOREIGN KEY (`id_compra`) REFERENCES `compra_r` (`id_compra`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK_PROD_DET` FOREIGN KEY (`id_prod`) REFERENCES `producto` (`id_prod`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=503 DEFAULT CHARSET=utf8 COLLATE=utf8_spanish2_ci;

-- Volcando datos para la tabla marketbbdd.producto_etiquetado: ~7 rows (aproximadamente)
...
INSERT INTO `producto_etiquetado` (`id_etiqueta`, `cod_barras`, `preciototal`, `peso_cantidad`, `estado`, `id_prod`, `id_compra`) VALUES
	...
	(481, 1052, 21, 7, 1, 52, 40),
	(482, 1081, 15, 2, 1, 81, 40),
	(483, 2002, 384, 8, 1, 2, 40),
	(490, 2002, 48, 1, 1, 2, 41);
